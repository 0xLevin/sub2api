...
export const METHOD_ORDER = ['alipay', 'alipay_direct', 'wxpay', 'wxpay_direct', 'stripe', 'airwallex', 'baonuo'] as const
...
export const WEBHOOK_PATHS: Record<string, string> = {
  ...
  baonuo: '/api/v1/payment/webhook/baonuo',
}
...
export const PROVIDER_CALLBACK_PATHS: Record<string, CallbackPaths> = {
  ...
  baonuo: { notifyUrl: WEBHOOK_PATHS.baonuo, returnUrl: RETURN_PATH },
...
export const PROVIDER_CONFIG_FIELDS: Record<string, ConfigFieldDef[]> = {
  ...
  baonuo: [
    ...
    { key: 'apiKey', label: '', sensitive: true },
    { key: 'apiBase', label: '', sensitive: false, defaultValue: 'https://baonuo.roseland.life', hintKey: 'admin.settings.payment.field_baonuoApiBaseHint' },
    { key: 'channelType', label: '', sensitive: false },
    { key: 'currency', label: '', sensitive: false, defaultValue: 'CNY', hintKey: 'admin.settings.payment.field_paymentCurrencyHint', options: PAYMENT_CURRENCY_OPTIONS },
  ],
}
