...
func TestOpenAISelectAccountWithLoadAwareness_StickyUsagePercentLimitClearsSession(t *testing.T) {
	sessionHash := "usage-percent-session"
	groupID := int64(1)
	now := time.Now()
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
	...
	if selection == nil || selection.Account == nil || selection.Account.ID != 2 {
		t.Fatalf("expected account 2 after hard usage percent limit, got %+v", selection)
	}
	if cache.deletedSessions["openai:"+sessionHash] != 1 {
		t.Fatalf("expected sticky session to be deleted")
	}
